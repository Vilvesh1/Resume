...
public class AngryCircles { // this program creates a game where you shoot a target 
  ...
  public static int targetBottom = 125;
  ...
  public static void drawData() { // this method draws the cannon, cannonball, and target. It also prints the creator of the game and the title, the cannon angle, target position, amount of force, time in seconds, hits, and score.
    ...
    g.drawString("Target position: " + targetTop +" - "+ targetBottom, 10, 60);
    ...
    drawTarget(xspot,targetTop,targetBottom,Color.BLACK);
  ...
  public static void targetUp() { // this moves the target up with the up key
    ...
    targetBottom = (targetBottom - 1);
    if (targetTop == 0 || targetBottom == 50)
    {targetTop = 250;
      targetBottom = 300;
    }
  }
  ...
  public static void targetDown() { // this moves the target down with the downkey key
    ...
    targetTop = (targetTop + 1);
    targetBottom = (targetBottom + 1);
    if (targetTop == 250|| targetBottom == 300)
    {targetTop = 0;
      targetBottom = 50;
    }
  }
  
  public static void drawTarget(int xspot, int targetTop, int targetBottom, Color c) { // this method draws the target
    g.setColor(c);
    g.drawLine(xspot,targetTop,xspot,targetBottom);
  }
  ...
  public static void drawCannonball(int x, int y, Color c) { // This method does the math to get the cannon ball postion and also draws the cannon ball
    x_2 = (x2 - (cr/2));
    y_2 = (y2 - (cr/2));
  ...
  public static int checkForHitOrMiss(int x6, int y6, int cr) { // This method checks if the cannon hit of missed the target
    if (x6 > 300 || y6 > 300 || x6 == 0 || y6 == 0 || (x6 >= 269 && y6 >= targetTop && y6 <= targetBottom))
    {
      if ((x6 >= 269 && y6 >= targetTop && y6 <= targetBottom))
      {x6 = x_2;
        y6 = y_2;
        g.setColor(Color.RED);
        g.fillOval(x6,y6,cr,cr);
  ...
  public static void keyDown(char c) { // this method shows the history by pressing the h key and resets the game by pressing r
