...
void loop() { // loops indefinitely
  ...
  while (sensorValue4 == 0) { // this loop puts the bot in a infinite loop which puts the bot in a inactive state (off).
    ...
    while ((inches <= 5 || sensorValue2 == 0 || sensorValue5 == 0) && sensorValue3 == 1) { // this loop makes the bot charge other bots motors (motors move forward)
         ...
         sensorValue3 = digitalRead(irPin2); 
          if (sensorValue3 == 0) // causes the bot to break out of loop due to line senor
         { 
         motor1.setSpeed(0); // motor 1 turns off
         motor2.setSpeed(0); // motor 2 turns off
         break;
    ...
    while ((inches > 5 || sensorValue2 == 1 || sensorValue5 == 1) && sensorValue3 == 1) { // this loop makes the bot spin to locate other bots (motors cause bot to rotate counterclockwise)
         motor1.run(FORWARD); // motor 1 moves forward
    ...
    while (sensorValue3 == 0 ) { // this loop makes the bot move backwards (line sensor sees white line) 
      if (i == 0) {
        delay(200);
        i++;
      }
